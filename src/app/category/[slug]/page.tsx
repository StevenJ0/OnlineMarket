...
const EmptyState = () => (
  <div className="text-center py-12 sm:py-20 bg-slate-900/60 rounded-3xl border border-slate-800 border-dashed">
    <ShoppingBag className="mx-auto text-slate-600 mb-4" size={48} />
    <p className="text-slate-300 mb-1 px-4">Belum ada produk di kategori ini.</p>
      ...
      <div className="p-4 space-y-2">
        ...
        {product.seller && (
          <p className="text-xs text-slate-400 truncate">
            Toko:{" "}
            <span className="text-slate-200">{product.seller.store_name}</span>
          </p>
        )}
        ...
        <div className="flex items-center gap-1 text-xs text-slate-400">
          <Star
            size={12}
            className={ratingAvg > 0 ? "text-yellow-500" : "text-slate-600"}
          />
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchByCategory = async () => {
      try {
        setLoading(true);
        // Fetch all products & filter client-side (Sesuai logika asli)
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-6">
            <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-400 flex-wrap">
              ...
              <span>/</span>
              <span className="text-slate-300 whitespace-nowrap">Kategori</span>
              <span>/</span>
              <span className="text-orange-400 font-medium break-words">{categoryName}</span>
            </div>

            <Link
              href="/"
              className="inline-flex items-center gap-2 text-xs sm:text-sm text-slate-300 hover:text-orange-400 whitespace-nowrap"
            >
              <ArrowLeft size={16} />
              Kembali ke Beranda
            </Link>
          ...
          <div className="flex flex-col gap-4 mt-4">
            <div>
              <span className="inline-block px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-orange-400 text-xs font-semibold mb-4">
                KATEGORI PRODUK
              </span>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white mb-3 break-words">
                {categoryName}
              </h1>
